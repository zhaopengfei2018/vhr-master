...
//                .setUserAgent(USER_AGENT)
     ...
     * @param headers    请求的头部信息
     */
    public static HttpResult sendPostForm(String url, File file, Map<String, String> parameters,
                                          Header[] headers, Cookie[] cookies, String charset) throws IOException {
        HttpResult httpResult = null;
        // 创建HttpPost
        ...
        if( file == null) {
            ...
            httpPost.setEntity(assemblyFormEntity(parameters, file));
        }
     ...
     */
    private static HttpEntity assemblyFormEntity(Map<String, String> parameters, File file) {
        MultipartEntityBuilder httpEntityBuilder = MultipartEntityBuilder.create();
        //添加文本
        if (parameters != null && !parameters.isEmpty()) {
            TrcLog.log(PubfuncBean.getLogfile(), "httpEntity添加parameter");
            for (String key : parameters.keySet()) {
                httpEntityBuilder.addTextBody(key, parameters.get(key));
            }
        }
        //添加文件
        if (file != null) {
            TrcLog.log(PubfuncBean.getLogfile(), "httpEntity添加file");
            FileBody fileBody = new FileBody(file);
            httpEntityBuilder.addPart("file", fileBody);
//            httpEntityBuilder.addBinaryBody("file", new FileInputStream(file));
        }
        //返回请求体
        return httpEntityBuilder.build();
    }
     ...
     * @param parameters 请求的表单参数
     */
    private static HttpEntity assemblyFormEntity(Map<String, String> parameters, String charset) throws UnsupportedEncodingException {
        //返回请求体
        return new UrlEncodedFormEntity(assemblyParameter(parameters), charset);
    }
